...
`Sender` 是用于聊天场景的输入框组件。具有丰富的交互功能和自定义选项。支持语音输入、清空输入内容、多种提交方式，并允许用户自定义头部、前缀和操作列表。组件还提供焦点控制、提交回调等功能，满足多样化的输入场景需求。
...
| `trigger`        | 指令弹窗变化时触发                                      | `interface TriggerEvent{oldValue: string; newValue: string; isOpen: boolean; }` |
